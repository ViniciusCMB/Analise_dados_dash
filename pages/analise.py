...
def calc_integral(x, y):
    ...
    print((1/(x.iloc[-1]-x.iloc[0]))*df_result.at[0, 'Valor'])
    from scipy import integrate
    integral = integrate.cumulative_simpson(y, x=x, initial=0)
    print((1/(x.iloc[-1]-x.iloc[0]))*integral[-1])
...
def update_graph(input_data, input_data2):
    ...
    ndf2['Tempo de Queima'] = ndf2['Tempo Bruto'] - \
        ndf2.at[input_data, 'Tempo Bruto']
    ...
    # popt, _ = curve_fit(objective, ndf['Tempo de Queima'], ndf['Força'])
    # a, b, c, d, e, f = popt

    # t = np.arange(min(ndf['Tempo de Queima']), max(ndf['Tempo de Queima']), 0.1)
    # pt = objective(t, a, b, c, d, e, f)
    ...
    fig.add_trace(go.Scatter(x=t, y=pt, mode='lines',
                  name='Ajuste Polinomial', line=dict(color='black', width=2)))
    fig.add_trace(go.Scatter(x=ndf['Tempo de Queima'], y=ndf['Força'], mode='markers', name='Dados Experimentais',
...
def download(input_data, input_data2, n_clicks, passw, nome):
    if passw == 1:
        if 'save' == ctx.triggered_id:
            ...
            fig['layout'].update(height=600, width=800, title=nome,
                                 xaxis_title='Tempo de Queima (s)', yaxis_title='Empuxo (N)', xaxis=dict(tickformat='.2f', dtick=0.5), legend=dict(
                                     orientation="h",
                                     yanchor="bottom",
                                     y=1.02,
